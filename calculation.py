...
def makeReactionsMatrix(bars, left, right):
    try:
        count = len(bars)
...
def makeReactionVector(bars, F_to_points, left, right):
    count = len(bars)
    knots = [0] * (count+1)
...
def calculateNx(bars, F_to_points, left, right):
    count = len(bars)
    A = makeReactionsMatrix(bars, left, right)
    B = makeReactionVector(bars, F_to_points, left, right)
    try:
        A = linalg.inv(A)
    except:
        linalg.lstsq(A, A, rcond=-1)
    v6 = numpy.dot(A,B)
...
def calculateUx(bars, F_to_points, left, right):
    count = len(bars)
    A = makeReactionsMatrix(bars, left, right)
    B = makeReactionVector(bars, F_to_points, left, right)
    try:
